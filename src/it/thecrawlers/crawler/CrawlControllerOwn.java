 ...
 *
 */
public class CrawlControllerOwn extends CrawlController  {
	...
	public CrawlControllerOwn(CrawlConfig config, PageFetcher pageFetcher,
	 ...
	 */
	public <T extends WebCrawler> void start(final Class<T> _c, final int numberOfCrawlers, CrawlHandler handler) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		this.start(_c, numberOfCrawlers, true,handler);
	}
	 ...
	 * @throws InstantiationException 
	 */
	public <T extends WebCrawler> void startNonBlocking(final Class<T> _c, final int numberOfCrawlers,CrawlHandler handler) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		this.start(_c, numberOfCrawlers, false,handler);
	}

	protected <T extends WebCrawler> void start(final Class<T> _c, final int numberOfCrawlers, boolean isBlocking,CrawlHandler handler) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		try {
			finished = false;
			...
			for (int i = 1; i <= numberOfCrawlers; i++) {
				Constructor <?>[] constructors=_c.getConstructors();
				@SuppressWarnings("unchecked")
				T crawler = (T) constructors[1].newInstance(handler);
			...
			final CrawlControllerOwn controller = this;
			final CrawlHandler handlerFinal=handler;
								...
								for (int i = 0; i < threads.size(); i++) {
									...
									if (!thread.isAlive()) {
										if (!shuttingDown) {
											...
											Constructor <?>[] constructors=_c.getConstructors();
											@SuppressWarnings("unchecked")
											T crawler = (T) constructors[0].newInstance(handlerFinal);
